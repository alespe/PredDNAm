...
source(file.path(dir_script_pred,"ConfusionMatrix_all.R"))
...
for(iteration in 1:N_iteration){
	...
	if(iteration==N_iteration){
		rownames(metrics_all) <- seq(1,N_iteration,1)
		colnames(metrics_all) <- c("accuracy","class.err","precision","sensitivity","specificity","f1score","auc")
		write.table(metrics_all,file.path(MY_DIR,j,Transformation,"Metrics_Results.txt"),sep="\t",row.names=TRUE,col.names=NA)
		write.table(labels_pred_merged,file.path(MY_DIR,j,Transformation,"All_predic_labels.txt"),sep="\t",row.names=TRUE,col.names=NA)

		rownames(metrics_all_train) <- seq(1,N_iteration,1)
		colnames(metrics_all_train) <- c("accuracy","class.err","precision","sensitivity","specificity","f1score","auc")
		write.table(metrics_all_train,file.path(MY_DIR,j,Transformation,"Metrics_Results_train.txt"),sep="\t",row.names=TRUE,col.names=NA)
...
png(file.path(MY_DIR,j,Transformation,'confusionMatrix_all.png'))	
conf_matrix_plot_all(labels_pred_merged,metrics_all,N_iteration)
dev.off()
png(file.path(MY_DIR,j,Transformation,'confusionMatrix_all_train.png'))	
conf_matrix_plot_all(labels_pred_merged_train,metrics_all_train,N_iteration)
dev.off()
